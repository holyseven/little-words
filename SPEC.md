...
- 不创建录音文件或 Blob URL，不上传声音；使用 Vosk Browser 的 WASM Worker 在本机识别主题词表，模型首次使用时下载并由 Service Worker 缓存。最终识别结果优先，逐词置信度只做温和提示，partial 仅在没有最终词时兜底；不显示星星或清晰度分数，不做音素标准度评分，也不写入单词掌握度。
