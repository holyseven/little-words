...
- 不创建录音文件或 Blob URL；家长设置默认尝试 Azure 音素评估，音频只通过同源安全代理发送本次短音频，代理密钥不进入网页包。在线服务未配置、离线或请求失败时自动回退 Vosk Browser 的 WASM Worker；也可以在家长设置中固定使用 Vosk。Vosk 的最终识别结果优先，逐词置信度只做温和提示，partial 仅在没有最终词时兜底。
- Azure 首版只用音素评估结果做宽松通过判断（准确度 45 分起，低于 70 分显示温和提示），不向孩子展示分数，不写入单词掌握度；阈值需用真实儿童样本继续校准。GitHub Pages 没有服务端代理，因此线上自动使用本地 Vosk；本地试用通过 `.env.local` 配置代理密钥。
