...
- Azure 首版只用目标词的音素评估结果做宽松通过判断（词准确度 60 分起，60–79 分或出现明显低分音素时显示温和提示但不推进游戏，80 分起且没有明显低分音素才触发庆祝）；本次在线反馈会列出综合、准确度、流畅度和完整度，不写入单词掌握度。阈值需用真实儿童样本继续校准。网页 key 对访问者可见，仅用于本项目的免费额度试用，不存放儿童录音。
