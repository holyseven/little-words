...
进度：M0 ✅ 已验收 · M1 ✅ 已验收 · M2–M4 ✅ 代码完成 · M5 ✅ 界面与运行打磨完成，桌面自动化通过；⏳ iPad / iPhone 真机最终验收和 A10 ≥50fps 实测待完成。当前版本 1.1.0；M6 ✅ 老师原始材料、课程每日任务与离线下载已接入，范围见第 18 节。
...
- 课程音频页提供跟读入口：先听老师原声，再用 `getUserMedia` + `AudioContext` 实时监听孩子的跟读，提供麦克风权限提示；不创建或保存录音文件。
- 录音只在当前页面内存中处理，离开页面释放 Blob URL 和麦克风轨道，不上传外部服务；使用麦克风需 HTTPS 或本机 localhost。
- 本版离线评估有效音量、连续发声比例和时长范围，反馈为 0–3 颗练习星和鼓励语。它衡量是否完成一次可用练习，不判断音素是否标准，也不写入主题单词掌握度。
- 精确发音评分需要本地语音/音素模型。后续先做真实 iPad Safari 技术验证，确认模型满足体积、耗电、延迟和儿童语音误判约束后，再决定是否加入。
