...
- 不创建录音文件或 Blob URL，不上传声音；使用 Vosk Browser 的 WASM Worker 在本机识别目标词，模型首次使用时下载并由 Service Worker 缓存。当前只判断识别文本是否包含目标词，不显示星星或清晰度分数，不做音素标准度评分，也不写入单词掌握度。
- 课本暂保留材料播放，隐藏旧整段跟读入口；未校对的课程材料不接入逐词练习。
- 精确发音评分仍需要音素级模型或强制对齐；Vosk 的词级识别不能单独证明发音标准。下一步先在真实 iPad Safari 验证 39MB 模型的首次下载、缓存、耗电、延迟和儿童语音误判，再决定是否进入正式练习。
