...
- Vosk 模式的声音只在内存中处理，不生成录音文件、不上传；Azure 模式会把本次最多 5 秒的短音频发给 Azure，评估完成后立即释放。切换单词、再次播放示范音、离开页面或切后台会释放麦克风；权限等待期间离开，随后到达的音轨也会立即关闭。
- 当前接入 Vosk Browser + 英文小模型（约 39MB），在浏览器 Worker 内离线识别主题词表；最终结果优先，逐词置信度只用于温和提示，partial 仅作兜底。它只能判断“听起来像不像目标词”，不等于音素标准度。Azure 在线评估会在本次反馈中列出综合、准确度、流畅度和完整度。
- 跟读评估保留两条通道：家长设置默认选择 **Azure 在线音素评估**，在线服务不可用时自动回退 Vosk；也可以切换为完全离线的 Vosk。Azure 只接收本次短音频，前端不保存录音。
- 本地或 GitHub Pages 试用 Azure：复制 `.env.example` 为 `.env.local`，填写 `VITE_AZURE_SPEECH_KEY` 和 `VITE_AZURE_SPEECH_REGION`，再运行 `npm run dev`。GitHub Actions 使用同名 Secrets 注入构建；这是本项目有意采用的限额试用方案，密钥会随网页代码对访问者可见。没有这些变量时才使用本地代理或回退 Vosk。
- 课本本次保留原始音视频播放，先隐藏旧整段跟读入口；逐词跟读须等待词表和句段校对。

**本轮验证（2026-09-18）**

- `npm test`：18 个文件、187 项测试通过；内容校验、TypeScript、生产构建通过。
...
改代码 → push → GitHub Actions 部署成功 → 各设备**联网启动或恢复前台**时检查并应用新版本。
Service Worker 用的是 `autoUpdate` + `skipWaiting`，不会弹「有新版本」的提示打扰孩子。
自动检查间隔至少 60 秒；家长页「关于与离线状态 → 检查更新」可以立即核对线上版本，并显示当前提交对应的版本标记。版本核对文件不进入 Service Worker 预缓存，避免 iPad Safari 一直拿旧版本回答“已是最新”。
...
**改了代码，iPad 上还是旧版本**
先确认 GitHub Actions 的部署已成功，再在联网时进入家长页「关于与离线状态 → 检查更新」。它会核对线上版本；如果新缓存尚未安装，会提示保持联网，Service Worker 控制权切换后自动刷新。核对版本标记是否对应最新提交。
如果旧版本还没有这个按钮，先在原来使用的 Safari 标签页刷新（主屏幕应用则完全关闭后重新打开），联网停留片刻让更新下载完成，再重新打开一次。
更新不需要删除主屏幕图标或清除网站数据；清除网站数据会丢失本机学习进度。
