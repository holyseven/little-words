...
- 课程音频页可进入「跟读这一段」：先听老师音频，再请求麦克风实时监听孩子的跟读，结束后立即丢弃声音数据。
- 不保存录音文件；反馈检查实时声音是否录到、连续发声比例和时长，并用鼓励语和 0–3 颗练习星提示下一次练习。
- 这不是音素级的「标准发音」判定。真正的离线发音评分需要额外的本地语音模型，后续先在真实 iPad 上验证模型体积、耗电、延迟和儿童语音误判率。
