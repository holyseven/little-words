...
- Vosk 模式的声音只在内存中处理，不生成录音文件、不上传；Azure 模式会把本次最多 5 秒的短音频发给配置好的代理，评估完成后立即释放。切换单词、再次播放示范音、离开页面或切后台会释放麦克风；权限等待期间离开，随后到达的音轨也会立即关闭。
...
- 本地试用 Azure：复制 `.env.example` 为 `.env.local`，填写 `AZURE_SPEECH_KEY` 和 `AZURE_SPEECH_REGION`，再运行 `npm run dev`。Vite 会在本机提供同源代理，密钥不会进入网页包；不要把密钥放进 `VITE_*` 变量，也不要提交 `.env.local`。GitHub Pages 没有代理，线上会自动使用 Vosk。
